...
-(TronTransaction *)signTransaction:(TronTransaction *)transaction
{
    ...
    //本地签名  不知道为什么有问题
        NSData *data2 = [NSJSONSerialization dataWithJSONObject:[transaction.raw_data keyValues] options:NSJSONWritingPrettyPrinted error:nil];
        NSData * data = [data2 SHA256Hash];
        transaction.signature = @[].mutableCopy;
        for (int i = 0 ; i < transaction.raw_data.contract.count; i++) {
            NSData *signData = [_crypto signatureForHash:data];
            [transaction.signature addObject:signData.hexString];
        }
...
-(void)refreshAccount:(void(^)(TronAccount *account, NSError *error))completion
{
    ...
    [TRXClient getAccount:account success:^(id  _Nonnull responseObject) {
...
- (void)getAccountResourceWithRequest:(void(^)(AccountResourceMessage *netMessage, NSError *error))completion
{
    TronAccount *account = [[TronAccount alloc]init];
    account.address = [self address];
    [TRXClient getAccountResource:account success:^(AccountResourceMessage * _Nonnull netMessage) {
        completion(netMessage,nil);
    }];
