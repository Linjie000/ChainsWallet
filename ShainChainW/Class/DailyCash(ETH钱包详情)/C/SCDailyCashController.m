...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([_wallet.ID isEqualToNumber:@(60)]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==1) {
        ...
        [se setReturnTextBlock:^(NSString *showText) {
            ...
            if ([selectStr isEqualToString:@"导出Keystore"]) {
                NSString *keyStore = [AESCrypt decrypt:wallet.keyStore password:showText];
                if ([RewardHelper isBlankString:keyStore]) {
                    [TKCommonTools showToast:LocalizedString(@"密码错误")];
                    return ;
                }
                SCExpKeyController *sc = [SCExpKeyController new];
                sc.keystoreStr = keyStore;
                [self.navigationController pushViewController:sc animated:YES];
            }
            if ([selectStr isEqualToString:@"导出私钥"]) {
