...
//    [self addSubview:_timeLabel];
    ...
    //箭头图片
    arrowImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"wallet_xiangxia"]];
    arrowImageView.frame = CGRectMake(width/2.0 , (FCXHandingOffsetHeight - 40)/2.0 + 5, 24, 24);
    arrowImageView.centerX = SCREEN_WIDTH/2;
    [self addSubview:arrowImageView];
    
    //转圈动画
    _activityView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    _activityView.color = MainColor;
...
- (void)layoutSubviews {
    ...
    arrowImageView.frame = CGRectMake(self.frame.size.width/2.0 - 100, (FCXHandingOffsetHeight - 40)/2.0 + 5, 24, 24);
    arrowImageView.centerX = SCREEN_WIDTH/2;
    _activityView.frame = arrowImageView.frame;
}
