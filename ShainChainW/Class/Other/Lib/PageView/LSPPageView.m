...
- (void)pageViewScollWillShowView:(id)pageView WithIndex:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(pageViewScollWillShowView:WithIndex:)]) {
        [self.delegate pageViewScollWillShowView:self WithIndex:index];
    }
}
