...
- (void)setModel:(coinModel *)model
{
    ...
    if (arr.count) {
        //冻结获取宽带
        NSDictionary *accountfrozen = arr[0];
        NSString *s = accountfrozen[@"frozen_balance"];
        _freezeLab.text = [NSString stringWithFormat:@"%.1f",[s floatValue]/kDense];
        ...
        if ([UserinfoModel shareManage].appDelegate.walletClient.account.account_resource) {
            ...
            NSDictionary *frozen_balanceaeng = (NSDictionary*)account_resource.frozen_balance_for_energy;
            NSString *seng = frozen_balanceaeng[@"frozen_balance"];
            _freezeLab.text = [NSString stringWithFormat:@"%.1f", [seng floatValue]/kDense+[s floatValue]/kDense];
        }
    }
