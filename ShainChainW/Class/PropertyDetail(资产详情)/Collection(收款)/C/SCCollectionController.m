...
- (void)setWallet:(walletModel *)wallet
{
    ...
    _addressLab.text = wallet.address;
...
//    [RewardHelper getColorHexWithWalletID:self.wallet.ID handle:^(NSString *colorHexString1, NSString *colorHexString2) {
//        dispatch_async(dispatch_get_main_queue(), ^{
//            self.gradientLayer.colors = @[(__bridge id)[UIColor colorFromHexString:colorHexString1].CGColor,
//                                          (__bridge id)[UIColor colorFromHexString:colorHexString2].CGColor];
//        });
//    }];
}
...
- (void)subViews
{
    ...
    _nameLab.size = CGSizeMake(self.width, 33);
    _nameLab.top = 24;
...
//    code.text = @"x8309794ihhrojwoejgroWAH-sgh_134";
    ...
    _addressControl.touchBlock = ^(YYControl *view, YYGestureRecognizerState state, NSSet *touches, UIEvent *event) {
        ...
        if (state==YYGestureRecognizerStateEnded) {
            [RewardHelper copyToPastboard:_addressLab.text];
        }
    };
...
@interface SCCollectionController ()
<SCCustomHeadViewDelegate>
@end
...
- (void)viewDidLoad {
    ...
    [backup setTapActionWithBlock:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            [RewardHelper shareSomthingImg:weakSelf.collectionHeadView.qrcode.image controller:self];
        });
    }];
}
