...
- (void)setNormalSelect:(NSString *)normalSelect
{
    _normalSelect = normalSelect;
    if ([_normalSelect integerValue]) {
...
- (void)setAddress:(NSString *)address
{
    ...
    if (_addressLab.width>180) {
        _addressLab.width = 180;
    }
    [self layoutIfNeeded];
}
...
- (void)subViews
{
    ...
    _cpImg = [YYControl new];
    _cpImg.size = CGSizeMake(13, 13);
    _cpImg.image = IMAGENAME(@"5.4复制");
    _cpImg.x = _addressLab.right + 3;
    _cpImg.centerY = _cpControl.height/2;
    [_cpControl addSubview:_cpImg];
