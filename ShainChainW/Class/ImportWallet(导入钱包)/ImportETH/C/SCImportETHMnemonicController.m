//
//  SCImportETHMnemonicController.m
//  SCWallet
//
...
@interface SCImportETHMnemonicController ()
<SCCustomPlaceHolderTextViewDelegate,UITextFieldDelegate>

@property(strong, nonatomic) UIButton *recoverBtn;
@property(strong, nonatomic) SCImportETHPathView *choosePath; //选择路径
...
@implementation SCImportETHMnemonicController
...
- (void)subViews
{
    ...
    //MARK: - 选择路径
    _choosePath = [SCImportETHPathView new];
    _choosePath.x = 0;
    _choosePath.top = _keystoreTF.bottom+18;
    [self.scrollView addSubview:_choosePath];
    [_choosePath setTapActionWithBlock:^{
        [weakSelf chooseWalletPath];
    }];
    ...
    [setpwdlab sizeToFit];
    setpwdlab.x = marginX;
    setpwdlab.y = _choosePath.bottom+15;
...
- (void)chooseWalletPath
{
    WeakSelf(weakSelf);
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    [SCAlertCreater addActionTarget:alert titles:@[LocalizedString(@"m/44'/60'/0'/0/0 默认")] color:SCColor(99, 154, 255) action:^(UIAlertAction * _Nonnull action) {
        if([action.title isEqualToString:LocalizedString(@"取消")])return ;
        NSArray *arr = [action.title componentsSeparatedByString:@" "];
        weakSelf.choosePath.pathLab.text = arr[0];
        weakSelf.choosePath.pathTypeLab.text = arr[1];
        SCLog(@"---- %@",action.title);
    }];
...
- (void)createIDAction
{
    NSString *mnemonics = _keystoreTF.text;
    ...
    [HSEther hs_inportMnemonics:mnemonics pwd:pwd block:^(NSString *address, NSString *keyStore, NSString *mnemonicPhrase, NSString *privateKey, BOOL suc, HSWalletError error) {
        if (suc) {
            ...
            wallet.isSystem = @(0);
            wallet.mnemonics = [AESCrypt encrypt:mnemonics password:pwd];;
            wallet.balance = @"";
            ...
            newWall.keyStore = [AESCrypt encrypt:keyStore password:pwd];
            newWall.address = address;
            newWall.mnemonics = [AESCrypt encrypt:mnemonicPhrase password:pwd];
            newWall.privateKey = [AESCrypt encrypt:privateKey password:pwd];
        ...
        {
            [SVProgressHUD dismiss];
            [TKCommonTools showToast:LocalizedString(@"导入失败，请确保助记词格式正确")];
        }
    }];
