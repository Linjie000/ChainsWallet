...
@interface ImportIOSTController ()
...
@implementation ImportIOSTController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = LocalizedString(@"导入IOST钱包");
    [self subViews];
}
...
- (void)createPublicKeys{
    ...
    // 将用户导入的私钥生成公钥
    public_key_1_from_local = [IOSTClient iost_getPublicByPrivateKey:private];
    [self requestAccountsAccordingPublicKey];
}

- (void)requestAccountsAccordingPublicKey{
    WeakSelf(weakSelf);
    [IOSTClient iost_getAccountMessageWithPublicKey:public_key_1_from_local handle:^(NSArray *models, BOOL success) {
        if (success) {
            //MARK:--  暂时不清楚多个账号的情况
            [self createIDAction:models[0]];
        }else{
            [TKCommonTools showToast:@"查询账号失败"];
...
#pragma mark - 导入
- (void)createIDAction:(IOSTAccounts *)account
{
    NSString *private = _keystoreTF.text;
    NSString *pwd = _pwdTF.text;
    NSString *repwd = _repwdTF.text;
    ...
    NSArray *walletArr= [walletModel bg_find:nil where:[NSString stringWithFormat:@"where %@=%@",[NSObject bg_sqlKey:@"ID"],[NSObject bg_sqlValue:@(291)]]];
    ...
    wallet.isSystem = @(0);
    wallet.mnemonics = @"";
    wallet.balance = account.account_info.balance;
    wallet.tips = tip;
    wallet.address = account.account_info.name;
    wallet.privateKey = privateKey_text;
    wallet.password = [AESCrypt encrypt:pwd password:pwd];
    if (!walletArr.count) {
        wallet.name = @"IOST-Wallet";
    }else
    {
        wallet.name = [NSString stringWithFormat:@"IOST-Wallet-%ld",walletArr.count];
    }
    Items *owerItem = account.account_info.permissions.owner.items[0];
    Items *activeItem = account.account_info.permissions.active.items[0];
    if ([owerItem.ids containsString:public_key_1_from_local] && ![activeItem.ids containsString:public_key_1_from_local]) {//只owner匹配
    ...
    }else if (![owerItem.ids containsString:public_key_1_from_local] && [activeItem.ids containsString:public_key_1_from_local]){// //只active匹配
    ...
    }else if ([owerItem.ids containsString:public_key_1_from_local] && [activeItem.ids containsString:public_key_1_from_local]){//owner和active全部匹配
        wallet.account_owner_public_key = public_key_1_from_local;
        wallet.account_owner_private_key = privateKey_text;
        wallet.account_active_public_key = public_key_1_from_local;
        wallet.account_active_private_key = privateKey_text;
