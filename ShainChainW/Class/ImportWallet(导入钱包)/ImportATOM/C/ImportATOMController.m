//
//  ImportATOMController.m
//  ShainChainW
//
//  Created by 林衍杰 on 2019/12/16.
...
@interface ImportATOMController ()
...
@implementation ImportATOMController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = LocalizedString(@"导入ATOM钱包");
    [self subViews];
}
...
- (void)subViews
{
    ...
    _keystoreTF.size = CGSizeMake(SCREEN_WIDTH-2*marginX, 88);
    ...
    _pwdTF.attributedPlaceholder = [[NSAttributedString alloc] initWithString:LocalizedString(@"钱包密码(8位以上字母数字组合)") attributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:SCGray(128)}];

    _pwdTF.size = CGSizeMake(view_width, 48);
    _pwdTF.x = marginX;
    ...
    _repwdTF.attributedPlaceholder = [[NSAttributedString alloc] initWithString:LocalizedString(@"重复输入密码") attributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:SCGray(128)}];
    _repwdTF.size = CGSizeMake(view_width, 48);
    _repwdTF.x = marginX;
    _repwdTF.tag = 257;
    ...
    _pwdtipTF.attributedPlaceholder = [[NSAttributedString alloc] initWithString:LocalizedString(@"密码提示信息") attributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:SCGray(128)}];
    _pwdtipTF.size = CGSizeMake(view_width, 48);
    _pwdtipTF.x = marginX;
    ...
    recoverBtn.top = _pwdtipTF.bottom + 30;
...
- (void)createIDAction
{
    NSString *keystor = _keystoreTF.text;
    NSString *pwd = _pwdTF.text;
    NSString *repwd = _repwdTF.text;
    NSString *tip = _pwdtipTF.text;
    if (![Account isValidMnemonicPhrase:keystor]) {
        [TKCommonTools showToast:LocalizedString(@"助记词格式错误")];
        return;
    }
    if ([RewardHelper isBlankString:keystor]) {
        [TKCommonTools showToast:LocalizedString(@"请输入助记词")];
        return;
    }
    ...
    const char* phraseStr = [keystor cStringUsingEncoding:NSUTF8StringEncoding];
    ...
    NSArray *walletArr= [walletModel bg_find:nil where:[NSString stringWithFormat:@"where %@=%@",[NSObject bg_sqlKey:@"ID"],[NSObject bg_sqlValue:@(118)]]];
    
    walletModel *wallet = [walletModel new];
    wallet.address = [NSString stringWithUTF8String:result.bytes];
    wallet.password=pwd;
    wallet.tips=tip;
    wallet.isHide=@"0";
    wallet.ID = @(118);
    wallet.portrait = [PortraitChoose getRandomPortraitName];
    wallet.name = [NSString stringWithFormat:@"ATOM-Wallet-%ld",walletArr.count];
    wallet.isSystem = @(0);
    wallet.mnemonics = [AESCrypt encrypt:keystor password:pwd];
    wallet.balance = @"";
    ...
    newWall.privateKey = [AESCrypt encrypt:privateKey.string password:pwd];
    newWall.mnemonics = [AESCrypt encrypt:keystor password:pwd];
