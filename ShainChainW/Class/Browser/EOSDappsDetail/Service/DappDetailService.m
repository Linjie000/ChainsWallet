...
@property(nonatomic , strong) GetEOSAccountRequest *getAccountRequest;
...
- (GetEOSAccountRequest *)getAccountRequest{
    if (!_getAccountRequest) {
        _getAccountRequest = [[GetEOSAccountRequest alloc] init];
    }
...
- (void)getEosAccountInfo:(CompleteBlock)complete{
    self.getAccountRequest.account_name = VALIDATE_STRING(self.choosedAccountName) ;
    [self.getAccountRequest postRequestDataSuccess:^(id DAO, id data) {
...
//#define eosnewyork_BASEURL @"https://api.eosnewyork.io"
//#define eospark_BASEURL @"https://api.eospark.com"
//#define eosinfra_BASEURL @"https://public.eosinfra.io"
        
        NSString *url = [NSString stringWithFormat:@"%@/v1/chain/abi_bin_to_json", @"https://public.eosinfra.io"];
        ...
        [outerNetworkingManager POST: url parameters: [self.abi_bin_to_jsonRequest parameters] progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            Abi_bin_to_json_Result *abi_bin_to_json_result = [Abi_bin_to_json_Result new];
            abi_bin_to_json_result.data = [Abi_bin_to_json mj_objectWithKeyValues:responseObject];
