...
- (NSString *)excuteMultipleActionsForScatterWithScatterResult:(ScatterResult_type_requestSignature *)scatterResult andAvailable_keysArray:(NSArray *)available_keysArray andPassword:(NSString *)password{
    ...
    walletModel *wallet = [EOSOperationManager shareManage].wallet;
...
#pragma mark -- [@"data"]
            EOSBlockChainInfo *model = [EOSBlockChainInfo mj_objectWithKeyValues:data[@"data"]];// [@"data"]
...
//        NSString *url = [NSString stringWithFormat:@"%@/api_oc_blockchain-v1.3.0/abi_json_to_bin", REQUEST_HTTP_BASEURL];
        NSString *url = [NSString stringWithFormat:@"%@/v1/chain/abi_json_to_bin", @"https://public.eosinfra.io"];
        SCLog(@"abi_json_to_binRequest url%@", url);
        SCLog(@"abi_json_to_binRequest param%@", [[self.abi_json_to_binRequest parameters] mj_JSONString]);
        [outerNetworkingManager POST: url parameters: [self.abi_json_to_binRequest parameters] progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            Abi_json_to_bin_Result *abi_json_to_bin_result = [Abi_json_to_bin_Result new];
            Abi_json_to_bin *result = [Abi_json_to_bin new];
            result.binargs = responseObject[@"binargs"];
            abi_json_to_bin_result.data = result;
...
//            }else{
//                [TKCommonTools showToast:abi_json_to_bin_result.message];
//            }
...
#pragma mark -- [@"data"]
        if ([data[@"code"] isEqualToNumber:@0 ]) {
            ...
            [TKCommonTools showToast:data[@"message"]];
        }
    } failure:^(id DAO, NSError *error) {
...
- (void)pushTransactionRequestOperation{
    walletModel *accountInfo = [EOSOperationManager shareManage].wallet;
    NSString *wif;
    if ([accountInfo.account_owner_public_key isEqualToString:self.required_Publickey]) {
        wif = [AESCrypt decrypt:accountInfo.account_owner_private_key password:self.password];
    }else if ([accountInfo.account_active_public_key isEqualToString:self.required_Publickey]) {
    ...
    //     [NSObject out_Int8_t:private_key andLength:32];
    if (!private_key) {
        [TKCommonTools showToast:LocalizedString(@"private_key can't be nil!")];
        return;
    }
