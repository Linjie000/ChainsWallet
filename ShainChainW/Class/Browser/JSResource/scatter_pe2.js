function _sendScRequest(serialNumber, params, methodName) {
    if (navigator.userAgent == 'ShineChainAndroid') {
        window.DappJsBridge.pushMessage(serialNumber, params, methodName)
    } else if (navigator.userAgent == 'ShineChain') {
...
class pefun {
    constructor() {}
    requestMsgSignature(params) {
        const serialNumber = serialNumberFn();
        _sendScRequest(serialNumber, params, 'requestMsgSignature');
    ...
    requestSignature(params) {
        const serialNumber = serialNumberFn();
        _sendScRequest(serialNumber, params, 'requestSignature');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    getAppInfo() {
        const serialNumber = serialNumberFn();
        _sendScRequest(serialNumber, '', 'getAppInfo');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    walletLanguage() {
        const serialNumber = serialNumberFn();
        _sendScRequest(serialNumber, '', 'walletLanguage');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    getEosAccount() {
        const serialNumber = serialNumberFn();
        _sendScRequest(serialNumber, '', 'getEosAccount');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    getWalletWithAccount() {
        const serialNumber = serialNumberFn();
        _sendScRequest(serialNumber, '', 'getWalletWithAccount');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    getEosBalance(params) {
        ...
        _sendScRequest(serialNumber, Jparams, 'getEosBalance');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    getEosAccountInfo(params) {
        const serialNumber = serialNumberFn();
        const Jparams = JSON.stringify(params);
        _sendScRequest(serialNumber, Jparams, 'getEosAccountInfo');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    getTransactionById(params) {
        const serialNumber = serialNumberFn();
        const Jparams = JSON.stringify(params);
        _sendScRequest(serialNumber, Jparams, 'getTransactionById');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    pushActions(params) {
        ...
        const Jparams = JSON.stringify(params);
        _sendScRequest(serialNumber, Jparams, 'pushActions');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
    ...
    pushTransfer(params) {
        const serialNumber = params.serialNumber;
        const Jparams = JSON.stringify(params);
        _sendScRequest(serialNumber, Jparams, 'pushTransfer');
        return new Promise((resolve, reject) = >{
                           window.callbackResult = function(returnSerialNumber, result) {
                           if (returnSerialNumber == serialNumber) {
                           resolve(JSON.parse(result))
...
class IdentityPE {
    constructor(account) {
        ...
        this.publicKey = 'EOS8NJX2UzUFvbAYH7y1KoZpAAP3zjincBaZnDsuvjQQ4VD1KRLeG';
        this.name = 'shinechainEOS';
        this.accounts = [{
                         name: account,
                         authority: 'active',
                         blockchain: 'eos'
...
class Index {
    ...
    getIdentity(requiredFields) {
        throwNoAuth();
        return new Promise((resolve, reject) = >{
                           pe.getEosAccount().then((res) = >{
                                                   var account = res.data;
                                                   const ids = new IdentityPE(account);
                                                   this.identity = ids;
                                                   resolve(ids)
                                                   })
                           })
    ...
    signatureProvider(...args) {
        ...
        return (network, _eos, _options = {}) = >{
            ...
            return proxy(_eos({
                         ...
                         var result = '';
                         var lostresult = await pe.requestSignature(JSON.stringify(signargs)).then((res) = >{
...
function inject() {
    ...
    window.ScatterJS.scatter.isExtension = true;
    window.ScatterJS.scatter.connect = () => new Promise(resolve => resolve(true));
}
inject();
