...
@interface UnicornBrowserController ()
...
@implementation UnicornBrowserController
...
- (void)viewDidLoad {
    ...
    //下拉刷新
    _refreshHeaderView = [self.mainScrollView addHeaderWithRefreshHandler:^(FCXRefreshBaseView *refreshView) {
        [weakSelf loadData];
    }];
    [_refreshHeaderView autoRefresh];
    
    [self addNotificationForName:KEY_SCWALLET_NETWORK_STATE response:^(NSDictionary * _Nonnull userInfo) {
        [weakSelf loadData];
    }];
}
...
- (void)dealloc
{
    [self removeNotificationForName:KEY_SCWALLET_NETWORK_STATE];
}
