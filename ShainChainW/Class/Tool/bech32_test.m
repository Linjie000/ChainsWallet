...
//            else
//            {
//                printf("bech32_encode success: '%s'\n  %s \n", valid_checksum[i],rebuild);
//                printf("------------ \n");
//            }
        }
    ...
    for (i = 0; i < sizeof(valid_address) / sizeof(valid_address[0]); ++i) {
        ...
        NSString *str_From_buff = [NSString stringWithCString:(char*)witprog encoding:NSUTF8StringEncoding];
        printf("segwit_addr_decode witprog: '%s'\n", str_From_buff);
        if (!ret) {
            hrp = "tb";
