...
+(void)hs_sendToAssress:(NSString *)toAddress money:(NSString *)money tokenETH:(NSString *)tokenETH decimal:(NSString *)decimal currentKeyStore:(NSString *)keyStore pwd:(NSString *)pwd gasPrice:(NSString *)gasPrice gasLimit:(NSString *)gasLimit block:(void(^)(NSString *hashStr,BOOL suc,HSWalletError error))block{
    ...
    __block EtherscanProvider *e = [[EtherscanProvider alloc]initWithChainId:ChainIdHomestead apiKey:@"77AKCF8ZFHFG9DJ2ZQV67XTEH3381P3V6E"];
...
//                            transaction.value = b;
                            ...
                            [[e sendTransaction:signedTransaction] onCompletion:^(HashPromise *pro) {
                                SCLog(@"CloudKeychainSigner: Sent - signed=%@ hash=%@ error=%@", signedTransaction, pro.value, pro.error);
                                if (pro.error == nil){
                                    SCLog(@"\n---------------【生成转账交易成功！！！！】--------------\n哈希值 = %@\n",transaction.transactionHash.hexString);
                                    SCLog(@" 7成功 哈希值 =  %@",pro.value.hexString);
                                    block(pro.value.hexString,YES,HSWalletSucSend);
                                    [[e getTransaction:pro.value]onCompletion:^(TransactionInfoPromise *info) {
                                        if (info.error == nil) {
                                            SCLog(@"===%@",info.value.transactionHash.hexString);
                                        }else{
                                            SCLog(@" 9查询哈希%@失败 %@",pro.value.hexString,pro.error);
                                        }
                                    }];
                                }else{
                                    SCLog(@" 8转账失败 %@",pro.error);
...
+(void)hs_getTransactionsByAddress:(NSString *)address
{
//
//    http://api.etherscan.io/api?module=account&action=txlist&address=0xddbd2b932c763ba5b1b7ae3b362eac3e8d40121a&startblock=0&endblock=99999999&sort=asc&apikey=77AKCF8ZFHFG9DJ2ZQV67XTEH3381P3V6E
    EtherscanProvider *e = [[EtherscanProvider alloc]initWithChainId:ChainIdHomestead];
    Address *a = [Address addressWithString:address];
    ArrayPromise *arr = [e getTransactions:a startBlockTag:1];
    SCLog(@"======= arr %@",arr);
}
