...
- (void)subViews
{
    ...
    titlelab.height = 40;
    titlelab.top = SCREEN_ADJUST_HEIGHT(132);
    titlelab.font = kPFFont(24);
    titlelab.textAlignment = NSTextAlignmentCenter;
    titlelab.text = LocalizedString(@"选择钱包类型");
    [self.view addSubview:titlelab];
    WeakSelf(weakSelf);
    NSArray *textarr = @[@"以太坊钱包",@"比特币钱包",@"EOS钱包"];
    NSArray *imgarr = @[@"9.3以太坊钱包",@"9.3比特币钱包",@"9.3EOS钱包"];
    for (int i=0 ;i<textarr.count;i++) {
        YYControl *view = [self createWalletType:textarr[i] coinImg:IMAGENAME(imgarr[i])];
        view.centerX = SCREEN_WIDTH/2;
        view.top = i*20+i*view.height+SCREEN_ADJUST_HEIGHT(74)+titlelab.bottom;
